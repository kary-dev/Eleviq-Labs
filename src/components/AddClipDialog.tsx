...
export function AddClipDialog({
  ...
  const platformRef = useRef(platform); // Track current platform to prevent stale fetch results
  ...
  const doFetch = (value: string) => {
    ...
    setError("");
    const fetchPlatform = platformRef.current; // Capture at fetch time
    startFetch(async () => {
      const p = await fetchClipPreview(fetchPlatform, v);
      // Only update if user is still on same platform (prevent stale result after switch)
      if (platformRef.current === fetchPlatform) {
        setPreview(p);
      }
    });
  };
